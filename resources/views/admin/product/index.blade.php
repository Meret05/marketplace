    ...
    <div class="relative overflow-x-auto">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
            ...
            <tr>
                ...
                <th class="px-6 py-3">{{ __('admin.sku') }}</th>
                <th class="px-6 py-3">{{ __('admin.actions') }}</th>
                <th class="px-6 py-3">{{ __('admin.variations') }}</th>
            </tr>
            </thead>
            <tbody>
            @foreach($products as $product)
                <tr class="bg-white border-b border-gray-200">
                    ...
                    <td class="px-6 py-4"><a
                            href="{{ route('admin.products.show', $product->id) }}">{{ $product->title }}</a></td>
                    <td class="px-6 py-4"><a
                            href="{{ route('admin.products.show', $product->id) }}">{{ $product->sku }}</a></td>
                    <td class="px-6 py-4 flex">
                        <a href="{{ route('admin.variations.create', $product->id) }}">
                ...
                <tr id="variations-{{ $product->id }}" class="hidden">
                    <td colspan="4" class="px-6 py-4 bg-gray-50">
                        <table class="w-full text-sm text-left text-gray-500 ">
                            <thead class="text-gray-700 border-b bg-gray-50">
                            <tr>
                                <th class="px-4 py-2">{{ __('admin.variation-sku') }}</th>
                                <th class="px-4 py-2">{{ __('admin.price') }}</th>
                                <th class="px-4 py-2">{{ __('admin.stock') }}</th>
                            ...
                            @foreach($product->variations as $variation)
                                <tr class="bg-white border-b border-gray-200">
                                    ...
                                    <td class="px-6 py-4 flex">
                                        <form action="{{ route('admin.variations.destroy', $variation->id) }}"
                                              method="post">
                                            @method('delete')
                                            @csrf
                                            <button>
